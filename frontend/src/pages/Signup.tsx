...
import { Mail, Lock, Eye, EyeOff, User as UserIcon, ArrowLeftRight } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen flex flex-row-reverse">
      ...
      <div className="hidden lg:flex w-1/2 relative bg-secondary overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-t from-deep-navy via-deep-navy/30 to-transparent z-10" />
        <div
          className="absolute inset-0 opacity-20"
          style={{
            backgroundImage: 'radial-gradient(circle, #3cdae8 1.5px, transparent 1.5px)',
          ...
          <ul className="space-y-4">
            {[
              ...
              <li key={item} className="flex items-start gap-3">
                <span className="text-vibrant-green text-xl leading-none">✓</span>
                <span className="text-body-lg text-primary-fixed-dim">{item}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 bg-surface-white">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" fullWidth size="lg" variant="secondary" loading={loading} className="mt-2">
              Create Account
            </Button>
          </form>
          ...
          <div className="mt-6 grid grid-cols-2 gap-4">
            <button type="button" className="flex justify-center items-center gap-2 py-2.5 px-4 border border-outline-variant rounded-xl bg-surface-white hover:bg-surface-container-low transition-colors">
